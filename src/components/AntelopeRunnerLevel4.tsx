...
const QUALITY_LIMITS: Record<Quality, { maxParticles: number; maxSwirls: number; maxClouds: number; cloudPuffs: number; analyserSkip: number; blur: boolean }> = {
  ...
  high:   { maxParticles: 55, maxSwirls: 11, maxClouds: 4, cloudPuffs: 26, analyserSkip: 1, blur: true },
  medium: { maxParticles: 34, maxSwirls: 11, maxClouds: 3, cloudPuffs: 18, analyserSkip: 3, blur: true },
  low:    { maxParticles: 20, maxSwirls: 11, maxClouds: 2, cloudPuffs: 12, analyserSkip: 5, blur: false },
};
...
function RunnerGame() {
  ...
  function triggerLightning() {
    ...
    sequence.forEach(([t, v]) =>
        setTimeout(() => setLightning(v), t)
    );
}
  ...
  useEffect(() => {
    const stored = Number(window.localStorage.getItem("antelope-best-level4") ?? 0);
  ...
  useEffect(() => {
    const loop = (t: number) => {
      ...
      const raceProgress = Math.min(1, s.dist / RACE_LENGTH);
      ...
      if (s.y < -120) {
        ...
        setBest((b) => {
          ...
          window.localStorage.setItem("antelope-best-level4", String(nb));
          return nb;
        });
      ...
      s.t += dt;
        ...
        if (s.t >= s.nextLightning) {
        triggerLightning();
        s.nextLightning += 30 + Math.random() * 8;
        }
    ...
    s.analyserSkip += 1;
      ...
      if (s.swirlIn <= 0) {
        ...
        s.swirlIn = intense
        ? (0.19 + Math.random() * 0.33) * spawnBoost
        : (0.47 + Math.random() * 0.73) * spawnBoost;
      }
      s.swirls = s.swirls
        .map((sw) => {
